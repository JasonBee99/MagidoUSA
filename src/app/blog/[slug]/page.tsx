...
export default function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  const { breadcrumb } = blogConfig;

  // Sort by date descending — matches the default "Newest First" order on the blog index
    ...
    <div className="min-h-screen bg-[var(--color-bg)] text-[var(--color-text)]">
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center gap-2 text-sm">
          <Link href={breadcrumb.home.href} className="text-[var(--color-text-muted)] transition-colors hover:text-magido-orange">
            {breadcrumb.home.label}
          </Link>
          <ChevronRight className="h-3.5 w-3.5 text-[var(--color-text-muted)]" />
          <Link href="/blog" className="text-[var(--color-text-muted)] transition-colors hover:text-magido-orange">
            {breadcrumb.current}
          </Link>
          <ChevronRight className="h-3.5 w-3.5 text-[var(--color-text-muted)]" />
          <span className="truncate max-w-xs font-medium text-[var(--color-text)]">{post.title}</span>
        </div>
      </nav>
